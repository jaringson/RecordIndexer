	...
	private static void parseBatches(NodeList multibatchList, int projectId, ArrayList<Field> fields) throws DatabaseException {
		for(int i =0;i<multibatchList.getLength();i++){
			...
			for(int j =0;j<batchList.getLength();j++){
		        ...
		        parseRecords(batchElem.getElementsByTagName("records"), batchId, fields, j);
			}
			
		}
	}

	private static void parseRecords(NodeList multirecordList, int batchId, ArrayList<Field> fields,int rowNumber) throws DatabaseException {
